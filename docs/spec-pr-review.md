...
- **Review タブ**
  - ヘッダーに投票ボタン群(Approve / w/ suggestions / Wait for author / Reject / Reset)。
    現在の自分の投票を強調表示。
  - PR 説明文(第 2 弾で Markdown レンダリングに対応。後述)。
...
## 第 2 弾(2026-06-13 追加スコープ)

GitHub の Web PR ビューとのギャップ補填として以下を追加する。

- **差分内スレッド表示**: Files タブの差分で、行アンカー付きスレッドを該当行直下に
  インライン表示し、その場で返信・解決/再開できる
- **インラインコメント作成 UI**: 差分の行(target 側の行番号がある行)から新規スレッド
  を作成できる(`postPullRequestComment` の `filePath`/`rightLine` を使用)
- **Markdown レンダリング**: PR 説明文・コメント本文を `marked` + `DOMPurify` で
  サニタイズ済み HTML として描画。リンクは外部ブラウザで開く
- **コミット一覧タブ**: パネルに Commits タブを追加。
  `GET .../pullRequests/{id}/commits` を新コマンド `list_pull_request_commits` で取得
- **Side-by-side 差分切替**: unified ↔ split 表示のトグル(インラインスレッドは
  どちらのモードでも right 行に表示)
