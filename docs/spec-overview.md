...
- **作業項目のフォロー (ローカル watchlist)**: プレビューの星トグルで作業項目をローカルの
  ウォッチリストに追加/解除 (`azdodeck:workItems:follows`、localStorage)。公開の follow
  設定 REST API が無いためサーバ側の購読には連動しない、クライアント専用機能。
