...
- PR 同期: active PR とレビュー対象 PR の取得を重ね合わせ、CI ステータスは最新 50 件を後段で付与。
  取得上限に達した project の active PR は upsert のみに留め、欠落行の削除は行わない。
  レビュー対象 PR はページングして全件取得し、project 単位の失敗時は org レベルのレビューキャッシュ全置換を避ける。
- 作業項目同期: プロジェクト単位で並列。`System.ChangedDate` デルタ取得 (24h ごとにフル) は維持。
  取得上限に達した project/snapshot は upsert のみに留め、欠落行の削除は行わない。
...
- イベント: 同期完了で `sync:updated` (org_id + 完了スコープ)。
  Commit 同期では commit activity query を stale 化し、表示中の commit search は現在の条件で再実行する。
