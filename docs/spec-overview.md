...
リポジトリ閲覧中は `T` でファジーファイルファインダを開く (入力欄にフォーカス中は発火しない)。
`↑ ↓` で候補移動、`Enter` で選択ファイルを `CodeFileView` で開く、`Escape` で閉じて元のフォーカス
位置 (通常はツリーの行) へ戻る。矢印/Enter はピッカー内に閉じ込め、背後のツリーは反応しない。
