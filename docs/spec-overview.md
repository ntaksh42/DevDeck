...
| **Pull Request Search** | プロジェクト/リポジトリ/ステータスで PR を検索。ソート可能グリッド、列リサイズ、`C` で URL コピー。検索条件を名前付きの保存検索 (ローカル) として保存・再適用でき、コマンドパレットからも起動可能。 |
...
| **Work Item Search** | キーワード + プロジェクト/状態/種別での作業項目検索。全文検索 (FTS)。検索条件を保存検索として保存・再適用 (パレット起動対応)。 |
| **Commits** | キーワード/プロジェクト/リポジトリ/作者/ブランチ/期間でコミット検索。7d/30d/90d プリセット。関連 PR の遅延ルックアップ。検索条件を保存検索として保存・再適用 (パレット起動対応)。 |
