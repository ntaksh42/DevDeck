...
| **Commits** | 差分パネルに Unified / Side-by-side 表示切替を追加。両表示とも highlight.js による行単位の構文ハイライトを適用 (`highlightLines`、ファイル全体を一度ハイライトしてから行単位に分割するため複数行にまたがるトークンも正しく着色される。語単位の変更ハイライトと構文ハイライトは同一行では併用せず、変更箇所がある行は語単位ハイライトを優先)。プレビューにコミットへ Azure DevOps のアーティファクトリンク経由で紐づく Work Item を一覧表示し、クリックで Work Items 検索へ遷移 (`get_commit_work_items`、`vstfs:///Git/Commit/{projectGuid}%2F{repoGuid}%2F{commitSha}` を `_apis/wit/artifacturiquery` に問い合わせ、work item リンクが無い場合はパネル非表示)。一覧の PR 列はグリッドの可視ウィンドウ単位でまとめて取得し (`get_commit_pull_requests_batch`、リポジトリ単位でバッチ化)、行ごとの個別リクエストを避けてプレビューを開く前に件数を表示する。GitHub 接続では Work Item 相当のリンク機構が無いため `get_commit_work_items` は空配列、バッチ PR 取得もリポジトリ横断のバッチ API が無いため空配列を返し、行プレビューを開いた際の単発取得にフォールバックする。 |
...
- 移行: `src-tauri/src/db.rs` の `migrate()` が `PRAGMA user_version` を使用。
  **現行スキーマバージョン: 17** (v17 で `organizations.provider_kind` を追加)。
