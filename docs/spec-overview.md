...
| **Settings** | 組織設定 (PAT / Azure CLI)、通知設定、フォルダパス、グローバルホットキー、キーバインド上書き。組織ごとの認証情報ヘルス (`check_organization_credential` で接続を probe し Connected / Re-authenticate / Unreachable を表示、シークレットは非表示)。Software update パネル (opt-in: 手動で更新確認→適用、失敗時は安全にスキップ。`tauri-plugin-updater`、ブラウザでは無効)。 |
