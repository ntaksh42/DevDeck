...
| **Pull Request Search** | プロジェクト/リポジトリ/ステータスで PR を検索。ステータスは active/completed/abandoned を複数選択可（既定は active）。active はローカルキャッシュから、completed/abandoned はオンデマンドで Azure DevOps からプロジェクト横断取得しマージ。ソート可能グリッド、列リサイズ、`C` で URL コピー。 |
...
- PR の同期キャッシュは **active** PR のみを保持する。My Reviews などキャッシュ駆動の
  読み取り専用ビューは active を前提とする。PR Search は active をキャッシュから返しつつ、
  completed/abandoned はオンデマンドで API から取得する（UI のステータス選択肢は実際に
  バックエンドが返せる範囲に一致させる）。
