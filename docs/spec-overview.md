...
| **Code (Files) — Compare** | Compare タブはベース/ターゲットの 2 リビジョン（ブランチ/タグ/コミット SHA、いずれも独立して選択可）間の差分に対応。選択すると変更ファイル一覧（フォルダレベル diff、`compare_repo_revisions`）を表示し、1 ファイルを選ぶとその diff (`get_repo_revision_file_diff`) をドリルダウン表示。ファイルツリーで開いていたファイルが変更ファイル一覧に含まれる場合は自動選択。diff 表示は Unified/Side-by-side 切替、空白無視 (leading/trailing のみ、`diff` パッケージの `ignoreWhitespace`)、長い行の word-wrap トグルに対応 (`DiffView` 共有コンポーネント、`src/lib/diffView.ts` の `buildDiffLines`/`buildSideBySideRows` を共通利用)。タグ一覧は `list_repo_tags`。IPC: `list_repo_tags` / `compare_repo_revisions` / `get_repo_revision_file_diff`。GitHub 接続では Code Browse 全体が非対応のため同様に非対応。 |
