...
| **Commits** | キーワード/プロジェクト/リポジトリ/作者/ブランチ/期間でコミット検索。7d/30d/90d プリセット。関連 PR の遅延ルックアップ。サイドバーでは「Commits」セクション配下に `All Commits` (このビュー) と `My Commits` をまとめる (Pull Requests / Work Items と同じ折りたたみ式)。 |
