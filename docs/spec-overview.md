...
- デスクトップ通知: 設定が有効な場合のみ。初回スナップショットでは過去分を通知しない。
  ...
  新着コメントでスヌーズが期限前に復帰した場合、その復帰を引き起こしたコメントの
  通知は抑制する (PR は通常表示に戻り、以降の活動は通常どおり通知する)。
  期限切れによる復帰では通知を抑制しない。
...
`Ctrl+S` (保留変更の適用) はアプリが全ビューで確保するため、適用対象を持たない
ビューでも WebView 既定の「ページを保存」は抑止する。

Ctrl 系のショートカットは修飾キー付きでも文字キーと重なるため、テキスト入力・
リッチテキストエディタにフォーカスがある間は発火しない (フォーカス移動系に加え、
`Ctrl+E` 同期 / `Ctrl+,` 設定 / `Ctrl+\` サイドバー開閉も同様)。入力欄での
`Escape` は「その入力から抜ける」だけを行い、グリッドが無い画面でも背後のヘルプ
やコマンドパレットを閉じることはない。
