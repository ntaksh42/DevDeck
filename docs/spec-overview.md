...
| **Commits** | 一覧・プレビューに作者アバターを表示 (`searchCriteria.includeUserImageUrl` で取得した URL を `fetch_commit_avatar` 経由でプロキシ取得、失敗時はイニシャルにフォールバック)。プレビューに committer を author と区別して表示 (異なる場合のみ)、日時は相対表示 + 絶対時刻ツールチップ。プレビューに「このコミットを含むブランチ/タグ」を表示 (`get_commit_refs`。Azure DevOps に該当 API が無いため、ref tip 一致は無料判定、それ以外は Diffs API のマージベース判定を ref ごとに実行。候補数が多いリポジトリは上限 30 件で打ち切り `truncated` を返す)。 |
...
- 移行: `src-tauri/src/db/migrate.rs` の `migrate()` が `PRAGMA user_version` を使用。
  **現行スキーマバージョン: 18** (v18 で `commits.committer_name` / `committer_email` /
  `committer_date` / `author_image_url` を追加)。
