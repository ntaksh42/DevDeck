...
| ビュー | 用途 |
|---|---|
| **My Reviews** | 自分がレビュアーの PR。投票状態・マージコンフリクト/CI バッジ・stale 強調・ローカルの done/archive トリアージ・ローカルのレビュー結果プレビュー。プレビューヘッダーに変更ファイル数・構成サマリーと、差分を開く前の規模/リスクマーカー (Large / No tests / Sensitive) を表示。 |
