...
| **Pull Request Search** | プロジェクト/リポジトリ/ステータス (active/completed/abandoned、いずれも複数選択可。空=既定 active)/ターゲットブランチ/期間 (作成日 or 完了日基準)/ドラフト除外で PR を検索。並び替え (作成日・完了日・タイトル)。active はキャッシュ、それ以外はライブ取得 (ターゲットブランチ・期間はサーバ側フィルタ)。結果は先頭 100 件で打ち切り、超過時はインジケータ表示。ソート可能グリッド、列リサイズ、`C` で URL コピー。Labels 列はキャッシュ済み PR とライブ取得 PR の両方で `GitPullRequest.labels` から表示され、ラベルによる複数選択フィルタ (`MultiSelectFilter`、いずれか一致) が付く。プロジェクト+リポジトリを 1 件だけ選択すると「New pull request」(`create_pull_request`、ソース/ターゲットブランチ・タイトル・説明を指定) と「Branches」パネル (`list_branch_summaries`、各ブランチの最終更新/ahead-behind/紐づく active PR、ブランチからの PR 作成へのショートカット) を利用できる。 |
...
| **Code (Files)** | リポジトリ + ブランチを選び、左のファイルツリーで階層を辿って閲覧する Azure DevOps Repos > Files 準拠ビュー。フォルダ選択時は中身を表 (Name / Last change / Commits、`latestProcessedChange=true` で各行の最新コミットを 1 リクエストで取得) で表示し、README.md があれば下に Markdown 描画。ファイル選択時は行番号付き + highlight.js による構文ハイライトで内容表示。Contents / History / Compare タブを切替: History は選択パスのコミット履歴 (`list_repo_history`、`searchCriteria.itemPath`)、Compare はベースブランチを選び現ブランチとの差分を共有 diff (`buildDiffLines` + `DiffLineText`) で表示。ファイルには「Find in file」(Ctrl+F、マッチ件数 + 前後ナビ) と「Blame」(公開 REST に行単位 blame API が無いため AzDO Web の blame ビューへリンク) を提供。左の検索ボックスは入力でツリーをファイル名フィルタ、Enter でリポジトリ内全文検索 (`search_code`) を実行し結果から該当ファイルを開く。ツリーは1階層ずつ遅延取得 (`list_repo_tree`、`recursionLevel=OneLevel`)、矢印キーで移動/展開/折りたたみ。リポジトリはお気に入り (★) を localStorage に保存して先頭に並べ、前回開いたリポジトリ/ブランチを復元する (`codeBrowseStorage`)。IPC: `list_repo_branches` / `list_repo_tree` / `get_repo_file` / `list_repo_history`。 |
...
- **PR ラベル**: レビューパネルのヘッダーで PR のラベルを表示し、名前で付与・削除できる
  (`add_pull_request_label` / `remove_pull_request_label`)。ラベルは `get_pull_request_review` /
  `search_pull_requests` のどちらにも含まれ (Azure DevOps の `GitPullRequest.labels` から取得、追加の
  API 呼び出し不要)、PR Search のグリッド列と複数選択フィルタにも使われる。GitHub 接続では
...
- 移行: `src-tauri/src/db/migrate.rs` の `migrate()` が `PRAGMA user_version` を使用。
  **現行スキーマバージョン: 18** (v17 で `organizations.provider_kind` を追加、v18 で
  `pull_requests.labels` (JSON 配列の文字列) を追加しキャッシュ済み active PR もラベル表示/絞り込みに使えるようにした)。
...
の状態/種別/プロジェクト、Commits のプロジェクト/リポジトリ) も、単一選択の
