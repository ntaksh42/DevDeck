...
| **Code (Files)** | リポジトリ + ブランチを選び、左のファイルツリーで階層を辿って閲覧する Azure DevOps Repos > Files 準拠ビュー。フォルダ選択時は中身を表 (Name / Last change / Commits、`latestProcessedChange=true` で各行の最新コミットを 1 リクエストで取得) で表示し、README.md があれば下に Markdown 描画。ファイル選択時は行番号付き + highlight.js による構文ハイライトで内容表示。`.md`/`.markdown` ファイルを開いた場合は README と同じ Markdown 描画パイプラインでレンダリングしたビューを既定表示し、「Rendered / Raw」トグルで構文ハイライト済みソース表示に切替可能 (ファイル切替時は Rendered にリセット)。png/jpg/jpeg/gif/svg/webp/bmp/ico は拡張子判定でインライン画像プレビュー (`get_repo_file_binary` で認証付き取得した bytes を base64 → data URL 化、フロント側で MIME 判定。10MB 超は `tooLarge`)。バイナリ/プレビュー不可/サイズ超過のファイルは「Open in Azure DevOps」リンクで AzDO Web 上のファイルを開ける。Contents / History / Compare タブを切替: History は選択パスのコミット履歴 (`list_repo_history`、`searchCriteria.itemPath`)、Compare はベースブランチを選び現ブランチとの差分を共有 diff (`buildDiffLines` + `DiffLineText`) で表示。ファイルには「Find in file」(Ctrl+F、マッチ件数 + 前後ナビ、Rendered 表示中は非表示) と「Blame」(公開 REST に行単位 blame API が無いため AzDO Web の blame ビューへリンク) を提供。左の検索ボックスは入力でツリーをファイル名フィルタ、Enter でリポジトリ内全文検索 (`search_code`) を実行し結果から該当ファイルを開く。ツリーは1階層ずつ遅延取得 (`list_repo_tree`、`recursionLevel=OneLevel`)、矢印キーで移動/展開/折りたたみ。リポジトリはお気に入り (★) を localStorage に保存して先頭に並べ、前回開いたリポジトリ/ブランチを復元する (`codeBrowseStorage`)。IPC: `list_repo_branches` / `list_repo_tree` / `get_repo_file` / `get_repo_file_binary` / `list_repo_history`。 |
