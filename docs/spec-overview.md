...
| **Commits** | キーワード/プロジェクト/リポジトリ (複数選択可)/作者/ブランチ/期間でコミット検索。7d/30d/90d プリセット。関連 PR の遅延ルックアップ。結果が表示上限(100件)を超えた場合は `truncated`/`total` を返し「Showing N of M commits」と母数を明示。プレビューから対象ブランチを選んで cherry-pick / revert を実行可能（確認ダイアログ必須、`read_only_validation_mode_enabled` 有効時は拒否、Azure DevOps 専用で GitHub 接続では非表示）。 |
