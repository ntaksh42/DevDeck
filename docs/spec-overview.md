...
| **Pipelines** | ビルド実行をプロジェクト/定義/ブランチ/結果/状態で一覧。タイムライン・ログ末尾の表示、再実行・キャンセル。パイプラインを Watch (購読、localStorage、最大 100) すると実行履歴を常時追跡し、最新実行の開始/終了をデスクトップ通知 (`desktop_notifications_enabled` に従う)。実行詳細にテスト結果サマリー (合格/失敗/合計件数と失敗テスト一覧) を表示 (`get_pipeline_test_summary`、classic `_apis/test` runs+results)。タイムラインの失敗ステージを個別に再実行 (`retry_pipeline_stage`、stageRefName=timeline の identifier、確認ダイアログ必須)。 |
