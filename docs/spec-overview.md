...
| **Wiki** | Wiki ページをキーワード横断検索 (`search_wiki_pages`、Azure DevOps Search の `wikisearchresults`) し、選択ページの Markdown 本文をサニタイズしてプレビュー (`get_wiki_page`)。結果リストは ↑↓ / J K / Home / End で移動、Enter でプレビューにフォーカス、O / Ctrl+Enter でブラウザの該当ページを開く (編集はブラウザへジャンプ、アプリ内編集は非対応)。Azure DevOps 専用機能 (Wiki は GitHub に相当概念がない) で、`capabilities.wiki` が false のとき (GitHub 接続) はサイドバーのナビ項目を非表示にする (Pipelines と同じ方式)。 |
...
`git` (PR/コミット/リポジトリ)、`work_items`、`pipelines`、`code_search`、`wiki`
(Wiki 検索/ページ取得)、`pr_review` (スレッド/コメント/差分)、`pr_status` (CI 集約)、
`identity`、`auth`、`client`、`error`。
...
`V` Work Item Views / `C` Commits / `B` Pipelines / `D` Code / `K` Wiki / `S` Settings。
