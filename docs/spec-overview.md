...
> このドキュメントは、提案・未着手の機能カテゴリ仕様 (`spec-*.md`) とは異なり、
> **現時点で実装済みの動作**をまとめた「現状仕様」のスナップショットである。
> 個別の設計解説は [`docs/design/`](design/README.md) を、将来計画は各
> `spec-*` ファイルを参照。コードと差異が出たら本書を更新すること。
