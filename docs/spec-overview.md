...
| **Code (Files)** | リポジトリ + ブランチを選び、左のファイルツリーで階層を辿って閲覧する Azure DevOps Repos > Files 準拠ビュー。フォルダ選択時は中身を表 (Name / Last change / Commits、`latestProcessedChange=true` で各行の最新コミットを 1 リクエストで取得) で表示し、README.md があれば下に Markdown 描画。ファイル選択時は行番号付き + highlight.js による構文ハイライトで内容表示。行番号はクリック/Shift+クリック/Shift+矢印で範囲選択でき、選択範囲をハイライト表示。選択は URL ハッシュ (`#L10-L20`) に反映され、同インスタンスで最初に開いたファイルはハッシュから復元される。「Copy link」(Azure DevOps Web の `?path=&line=&lineEnd=` 形式パーマリンク) と「Copy lines」(選択行のテキストをコピー) を提供。Contents / History / Compare は roving tabindex の `tablist`/`tab`/`tabpanel` (矢印キーでフォーカス移動、Enter/Space で切替): History は選択パスのコミット履歴 (`list_repo_history`、`searchCriteria.itemPath`)、Compare はベースブランチを選び現ブランチとの差分を共有 diff (`buildDiffLines` + `DiffLineText`) で表示。ファイルには「Find in file」(Ctrl+F、マッチ件数 + 前後ナビ) と「Blame」(公開 REST に行単位 blame API が無いため AzDO Web の blame ビューへリンク) を提供。左の検索ボックスは入力でツリーをファイル名フィルタ、Enter でリポジトリ内全文検索 (`search_code`) を実行し結果から該当ファイルを開く。ツリーは1階層ずつ遅延取得 (`list_repo_tree`、`recursionLevel=OneLevel`)、矢印キー/Home/End/PageUp/PageDown で移動/展開/折りたたみ、頭文字の type-ahead で次の一致項目へジャンプ。ツリー幅はドラッグまたは `role="separator"` のハンドルを矢印キー/Home/End で操作してリサイズでき、localStorage に永続化 (`codeBrowseStorage`)。リポジトリはお気に入り (★) を localStorage に保存して先頭に並べ、前回開いたリポジトリ/ブランチを復元する (`codeBrowseStorage`)。IPC: `list_repo_branches` / `list_repo_tree` / `get_repo_file` / `list_repo_history`。 |
