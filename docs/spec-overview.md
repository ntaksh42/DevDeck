...
| **Commits** | グリッドで任意の 2 コミットを選択して比較ビューを表示 (`Space` で focused row をマーク、または `Shift+クリック`。2 件目を選ぶとプレビューが差分ビューに切替、`Esc` で解除)。変更ファイル一覧 + ファイル単位の差分は単一コミットの差分表示と同じ UI (`FileDiffView`) を再利用。IPC: `get_commit_range_changes` (Azure DevOps は `diffs/commits`、GitHub は compare API)。ファイル単位の内容取得は既存の `get_commit_file_diff` を base→`parentCommitId` / target→`commitId` として再利用。異なるリポジトリ間の 2 コミットは比較不可 (メッセージ表示のみ)。 |
...
Commits グリッドでは `Space` (focused row) または `Shift+クリック` で 2 コミットを比較用に
マーク (3 件目で最古のマークを解除)、2 件揃うとプレビューが比較ビューに切替、`Esc` で解除。
