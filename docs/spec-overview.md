...
| **PR レビュー** | スレッド/コメント/差分の表示と投票・スレッド解決・コメント投稿/編集/削除。説明とコメントは `MarkdownView` で描画。`get_pull_request_review` は説明・コメント内の `@<guid>` メンションを `mention_display_names` (id→表示名) として解決して返し、プレビューが生 id ではなく表示名 (`@名前`) を表示する。コメント投稿時は選択したメンションの `@名前` を `@<guid>` マークダウンへ変換し、Azure DevOps 側で実メンション (通知付き) になるようにする。 |
...
- **作業項目編集**: state / assignee / priority / フィールドをステージし 1 リクエストで適用。
  ...
  トークンはエスケープして可視テキストとして残す。メンション置換ロジックは作業項目とマークダウン
  (PR) の両プレビューで `src/lib/mentions.ts` を共有する。
