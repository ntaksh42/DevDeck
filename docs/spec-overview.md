...
| **Pipelines** | ビルド実行をプロジェクト/定義/ブランチ/結果/状態で一覧。タイムライン・ログ末尾の表示、再実行・キャンセル。実行詳細にテスト結果サマリー (合格/失敗/合計件数と失敗テスト一覧) を表示 (`get_pipeline_test_summary`、classic `_apis/test` runs+results)。 |
...
- **作業項目編集**: state / assignee / priority / フィールドをステージし 1 リクエストで適用。
  ...
  (`set_work_item_comment_reaction`、取得は `$expand=reactions`)。
