...
世の類似アプリや周辺ツールの機能を調べ、DevDeck の仕様案として使えるアイデアをできるだけ広く出す。

DevDeck の現在の軸は「Azure DevOps の Pull Requests / Work Items / Commits を desktop app で横断検索・監視する」こと。  
...
DevDeck では `Pull Requests > My Reviews` がすでにこの方向に近い。次は Work Items / Commits / Notifications も inbox 化できる。
...
DevDeck でも「No Vote」「Waiting for Author」などを固定タブだけで終わらせず、ユーザーが自分用の section を作れるようにすると強い。
...
DevDeck でも「一覧にある」だけではなく、「なぜ今見るべきか」を badge / section / alert にするべき。
...
Azure DevOps は PR と work item のリンク、commit と work item のリンクを持てる。DevDeck が desktop app として価値を出すなら、単体検索より「この変更はどの work item / PR / commit / build / deployment に繋がっているか」を即座に辿れることが強い。
...
| AI-06 | P2 | Query builder | natural language から DevDeck filter query |
...
- 個人 productivity metric は誤用されやすい。DevDeck は「自分が次に何をすべきか」を助ける設計を主軸にし、個人評価 dashboard に見える表現は避ける。
