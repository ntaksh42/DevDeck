...
export function CommitSearch({
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if ((branch.trim() || itemPath) && !repositoryId) {
      setValidationError(
        itemPath
          ? "Select a repository to filter commits by path (path: is applied on the server)."
          : "Select a repository to search a specific branch.",
      );
      return;
    }
    setValidationError(null);
